...
!declarações para registro de tempo da saída da GPU (overhead runtime)
real*8  :: t_inicio_exit, t_final_exit, tempo_exit_s
...
! Desmontagem da regiao da GPU (overhead sendo capturado)
t_inicio_exit = omp_get_wtime()
...
t_final_exit = omp_get_wtime()
tempo_exit_s = t_final_exit - t_inicio_exit
    ...
    ! Tempo da geracao dos ultimos I/Os do arquivo mestre
    t_final_escrita = omp_get_wtime()
    tempo_total_escrita_gpu_s = tempo_total_escrita_gpu_s + (t_final_escrita - t_inicio_escrita)
    
    ! Captura o wall clock total (Ate o final do processamento das variaveis)
    t_prog_fim = omp_get_wtime()
    tempo_total_programa_gpu_s = t_prog_fim - t_prog_inicio

    ! Escrevendo metricas reais isoladas 
    write(26, *) ""
    write(26, *) "===================================================="
    write(26, *) "              METRICAS DE PERFORMANCE               "
    write(26, *) "===================================================="
    write(26, '(A, F12.6, A)') "Tempo de execucao Parte 1 (GPU Count)   : ", t_parte1, " s"
    write(26, '(A, F12.6, A)') "Tempo de execucao Parte 2 (CPU Scan)    : ", t_parte2, " s"
    write(26, '(A, F12.6, A)') "Tempo de execucao Parte 3 (GPU Fill)    : ", t_parte3, " s"
    write(26, '(A, F12.6, A)') "Tempo Total da Topologia (1+2+3)        : ", tempo_total_sim_s1, " s"
    write(26, *) "----------------------------------------------------"
    write(26, '(A, F12.6, A)') "Tempo Acumulado BC (GPU)                : ", tempo_total_bc_gpu_s, " s"
    write(26, '(A, F12.6, A)') "Tempo Acumulado Forcas/Dano (GPU)       : ", tempo_total_forca_dano_gpu_s, " s"
    write(26, '(A, F12.6, A)') "Overhead Desmontagem GPU (Exit Data)    : ", tempo_exit_s, " s"
    write(26, '(A, F12.6, A)') "Tempo Total de Escrita (I/O)            : ", tempo_total_escrita_gpu_s, " s"
    write(26, *) "----------------------------------------------------"
    write(26, '(A, F12.6, A)') "Tempo Total (Wall-Clock)                : ", tempo_total_programa_gpu_s, " s"
