...
!______________________________DAQUI____________________________________________
    ...
    ! Application of boundary conditions at the top and bottom edges (Sequencial)
    ! NOTA DE DESEMPENHO: A tentativa prévia de paralelizar as BCs na GPU (via TARGET TEAMS DISTRIBUTE)
    ! resultou em aumento do tempo total de execução devido ao overhead de lançamento e sincronização 
    ! de kernels para poucos pontos materiais (~1.500 nós por borda). Por isso, foram mantidas sequenciais 
    ! na CPU, sincronizando apenas os dados via TARGET UPDATE TO.
    ...
    ! Atualiza a memória da GPU com os novos valores de contorno calculados sequencialmente na CPU
    !$omp target update to(vel((totint+1):tottop,2), disp((totint+1):tottop,2))
